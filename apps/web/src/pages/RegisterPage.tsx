...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err.message || 'Kaydolma sırasında hata oluştu.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        <div className="rounded-3xl border border-white/20 bg-white/10 p-8 backdrop-blur-xl">
          ...
          <div className="mb-8 text-center">
            <h2 className="text-3xl font-bold text-white">Yeni Hesap Oluştur</h2>
            <p className="mt-2 text-sm text-indigo-200">Başlamak için bir hesap oluşturun</p>
          </div>

          {/* Form */}
          <form onSubmit={handleSubmit} className="space-y-4">
            <label className="block">
              <span className="text-sm font-medium text-white">Ad Soyad</span>
              <input
                value={name}
